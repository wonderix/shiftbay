...
CSV.read(File.join(File.dirname(__FILE__),"users.csv"),col_sep: ";", encoding: "windows-1252").each do | row |
  firstname, lastname = row[0].strip.split(/\s+/)
  lastname = lastname.join(" ") if lastname.is_a?(Array)
  user = User.create :firstname => firstname, :lastname => lastname, :email => "#{firstname}.#{lastname}@sonnenhof.de", :level_of_employment => row[1].to_f/100, :job_title => row[2], :qualification => ( row[2] =~ /ex/ ? ex :helper)
  row[3..-1].each do | i |
    next unless i 
    i.strip!
    next if i.empty?
